...
import { ECHARTS_FONT_FAMILY } from "../charts/chartColors";
import { PlatformIcon } from "../social/PlatformIcon";
import { assetUrl } from "../../lib/assetUrl";
import { formatNumber } from "../../lib/formatters";
import "../../styles/bubble-pool-canvas.css";
...
export interface BubblePoolItem {
  accountLabel: string;
...
function nodeRadius(audienceCount: number | null) {
  return 26 + Math.min(20, Math.log10((audienceCount ?? 0) + 1) * 4);
}
...
function buildNodes(items: readonly BubblePoolItem[], categories: PoolCategory[]) {
  const indexByLabel = new Map(categories.map((category, index) => [category.label, index]));
  const seats = new Map<number, number>();

  return items.map((item) => {
    const categoryIndex = indexByLabel.get(item.categoryLabel) ?? 0;
    ...
    return {
      ...
      vx: 0,
      vy: 0,
      r: nodeRadius(item.audienceCount),
      focus: 0,
      phase: ((item.id % 100) / 100) * Math.PI * 2,
      orbit,
      categoryIndex,
      item,
    } satisfies PoolNode;
...
function loadImages(nodes: PoolNode[]) {
  ...
  nodes.forEach((node) => {
    const source = assetUrl(node.item.profileImageUrl ?? "");
    if (!source) return;
    const image = new Image();
    image.decoding = "async";
    image.onload = () => images.set(node.item.id, image);
    image.src = source;
  });
  return images;
}
...
function platformMark(platform: BubblePoolItem["platform"]) {
  const key = platform === "YOUTUBE" ? "YOUTUBE" : "INSTAGRAM";
...
function drawSelection(
  ...
  const shellRadius = bubbleShellRadius(radius);
  const badgeX = x + shellRadius * 0.68;
  const badgeY = y - shellRadius * 0.68;

  context.save();
  context.strokeStyle = `rgb(${TEAL})`;
  context.lineWidth = 3;
  context.beginPath();
  context.arc(x, y, shellRadius + 3, 0, Math.PI * 2);
  context.stroke();

  context.fillStyle = `rgb(${TEAL})`;
  context.beginPath();
  context.arc(badgeX, badgeY, 10, 0, Math.PI * 2);
  context.fill();
  context.fillStyle = "#fff";
  context.font = `800 ${CANVAS_LABEL_SIZE}px ${CANVAS_FONT}`;
...
/** 프로필 이미지가 없거나 깨지면 공통 사용자 아이콘을 보여준다. */
function PoolAvatar({ item }: { item: BubblePoolItem }) {
  const [failed, setFailed] = useState(false);
  const source = assetUrl(item.profileImageUrl ?? "");

  return (
    <span className="hsas-selector-pool__dock-photo">
      {source && !failed ? (
        <img
          alt={`${item.dockTitle} 프로필 이미지`}
...
export interface BubblePoolCanvasProps {
  emptyMessage?: string;
  itemNoun: string;
  items: readonly BubblePoolItem[];
  onActivate: (id: number) => void;
  onPrefetch?: (id: number) => void;
  selectedIds?: ReadonlySet<number>;
}

export function BubblePoolCanvas({
  emptyMessage,
  itemNoun,
  items,
  onActivate,
  ...
  const spotlightRef = useRef<((itemId: number | null) => void) | null>(null);
  // 도크를 훑는 동안에는 목록을 고정한다(화면이 움직여 목록이 바뀌는 되먹임 방지).
  const dockHoverRef = useRef(false);
  const [visibleState, setVisibleState] = useState(() => ({ source: items, value: items }));
  const visible = visibleState.source === items ? visibleState.value : items;
  const [focusState, setFocusState] = useState<{
    source: readonly BubblePoolItem[];
    value: string | null;
  }>(() => ({ source: items, value: null }));
  const focus = focusState.source === items ? focusState.value : null;
  ...
  useEffect(() => {
    ...
    const nodes = buildNodes(items, categories);
    ...
    spotlightRef.current = (id) => {
      spotlight = id == null ? null : nodes.find((node) => node.item.id === id) ?? null;
      if (!spotlight) return;
      const box = canvas.getBoundingClientRect();
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (nextHovered && nextHovered !== hovered) prefetchRef.current?.(nextHovered.item.id);
      hovered = nextHovered;
    ...
    const syncVisible = (time: number, width: number, height: number) => {
      ...
      const key = inView.map((item) => item.id).join(",");
      if (key === visibleKey) return;
      visibleKey = key;
      setVisibleState({ source: items, value: inView });
    };
    ...
    const render = (time: number) => {
      ...
      if (lit) {
        ...
        const name = lit.item.displayName;
        const account = lit.item.accountLabel;
        const audience = lit.item.audienceCount == null
          ? null
          : `${lit.item.audienceLabel} ${formatNumber(lit.item.audienceCount)}`;

        const mark = platformMark(lit.item.platform);
        ...
        const metaWidth = Math.max(
          accountWidth,
          audience ? context.measureText(audience).width : 0,
        );
        const cardWidth = Math.max(nameWidth, metaWidth) + 28;
        const cardHeight = audience ? 62 : 46;
        ...
        if (audience) context.fillText(audience, position.x, cardY + 50);
      }
    };
    frame = requestAnimationFrame(render);
  ...
  const focusCategory = (label: string | null) => {
    setFocusState({ source: items, value: label });
    cameraRef.current?.(label);
  };
  ...
  return (
    <div className="hsas-selector-pool">
      <canvas
        aria-label={`${itemNoun} 발견 풀`}
        className="hsas-selector-pool__canvas"
        ref={canvasRef}
        role="img"
      />
      {!items.length && emptyMessage ? (
        <p className="hsas-selector-pool__empty" role="status">{emptyMessage}</p>
      ) : null}
      ...
      >
        <button
          ...
          type="button"
        >
          <span>{dockOpen ? `현재 보이는 ${itemNoun} ${visible.length}명` : `${itemNoun} 목록 보기`}</span>
          ...
          <ul className="hsas-selector-pool__dock-list">
            {visible.map((item) => {
              ...
              return (
                <li key={item.id}>
                  <button
                    aria-label={selectionEnabled
                      ? `${item.displayName} ${selected ? "선택 해제" : "선택"}`
                      : undefined}
                    aria-pressed={selectionEnabled ? selected : undefined}
                    className={`hsas-selector-pool__dock-item${selected ? " is-selected" : ""}`}
                    onBlur={() => spotlightRef.current?.(null)}
                    onClick={() => activateRef.current(item.id)}
                    onFocus={() => {
                      spotlightRef.current?.(item.id);
                      prefetchRef.current?.(item.id);
                    }}
                    onMouseEnter={() => {
                      spotlightRef.current?.(item.id);
                      prefetchRef.current?.(item.id);
                    ...
                    type="button"
                  >
                    <PoolAvatar item={item} />
                    <span className="hsas-selector-pool__dock-text">
                      <strong>{item.dockTitle}</strong>
                      <span>
                        {item.platform ? (
                          <PlatformIcon
                            decorative
                            platform={item.platform === "YOUTUBE" ? "YouTube" : "Instagram"}
                          />
                        ) : null}
                        {item.dockSubtitle}
                      </span>
                    ...
                    {selected ? (
                      <span aria-hidden="true" className="hsas-selector-pool__dock-check">✓</span>
                    ) : null}
                  </button>
                </li>
              );
