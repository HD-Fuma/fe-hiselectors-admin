import { CONTENT_REVIEWS } from "../content/fixtures";
import { CREATORS } from "../creators/fixtures";
import { SETTLEMENTS, type PaymentStatus } from "../operations/fixtures";
import { CONTENT_INFLUENCE, campaignNameById } from "../performance/fixtures";
...
function createContentRecords(selector: SelectorFixture, sequence: number): SelectorContentRecord[] {
  ...
  const canonicalRecords: SelectorContentRecord[] = relatedContents.map((content, index) => {
    const review = CONTENT_REVIEWS.find((item) => item.id === content.id);
    ...
    return {
      ...
      format: review?.contentFormat ?? (content.platform === "YouTube" ? "유튜브 롱폼" : "인스타 피드"),
      id: content.id,
      publishedAt: review?.submittedAt ?? `${selector.recentActivity} 14:00`,
      status: review?.reviewStatus ?? "승인",
      thumbnailUrl: review?.currentSnapshot.mediaUrls[0]
        ?? featuredContent?.thumbnailUrl
        ?? fallbackThumbnail(sequence, index),
      title: review?.contentTitle ?? content.title,
    };
  });
