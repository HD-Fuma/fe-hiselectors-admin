...
`/creators`의 기본 화면은 사용자가 제공한 첫 번째 참고 이미지처럼 **한 사람의 프로필과 인기 콘텐츠가 먼저 보이는 카드**로 구성한다. 한 크리에이터 프로필은 `Instagram` 또는 `YouTube` 중 하나에만 속한다. 다중 플랫폼 배열과 Facebook 프로필은 제거한다.
...
카드 그리드는 데스크톱에서 3열, 1280px 미만의 기존 최소 셸 환경에서는 2열 계약을 유지한다. 별도 데모나 시각 표본 경로는 만들지 않는다.

## 3. 카드 정보 구조

카드는 아래 순서를 고정한다.

1. 같은 너비와 높이의 인기 콘텐츠 썸네일 3개를 한 줄로 배치한다. 각 모서리는 8px이며, 영상에는 작은 재생 표식만 둔다.
2. 76px 원형 프로필 이미지를 카드 중앙에 겹친다. 프로필 우측 아래에 Instagram 또는 YouTube 아이콘 하나만 표시한다.
...
각 크리에이터는 배열 대신 단일 `profile`을 가진다.
...
인기 콘텐츠는 해당 프로필의 플랫폼을 상속하므로 콘텐츠마다 플랫폼을 다시 선언하지 않는다. 각 항목은 `thumbnailUrl?: string`을 받을 수 있고, URL이 없거나 이미지 로드가 실패하면 기존 네트워크 없는 아트워크를 중립 fallback으로 사용한다.

목록 표와 상세 채널 표도 단일 `profile`만 사용한다. `cr-001`·`cr-003`은 Instagram, `cr-002`는 YouTube, `cr-004`는 Instagram으로 고정한다. 제안 채널은 기존 계약을 보존해 Instagram 프로필은 Instagram DM을 사용할 수 있고, YouTube 프로필은 이메일을 사용한다.
...
- 카드 반경: 14px, 그림자는 기본 상태에서 사용하지 않음
...
- 콘텐츠가 3개보다 적으면 있는 항목만 표시하며 레이아웃이 깨지지 않는다.
